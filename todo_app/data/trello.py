...
class Trello:
    ...
    def get_items(self) -> List[dict]:
        ...
        for card in json_response:
            ...
            list_id = card['idList'] 
            status = CardStatus.get_status(list_id, self.get_board_lists())
    ...
    def add_item(self, name: str, description: str) -> None:
        ...
        """
        endpoint = "cards"
        todo_list = next((list for list in self.get_board_lists() if list['name'] == CardStatus.TODO.value), None)
        params = {
            'key': self.app_key,
            'token': self.token,
            'name': name,
    ...
    def update_item(self, card_id: str, status: Enum) -> None:
        ...
        """
        endpoint = f"cards/{card_id}"
        done_list = next((list for list in self.get_board_lists() if list['name'] == status.value), None)
        params = {
            'key': self.app_key,
            'token': self.token,
            'id': card_id,
